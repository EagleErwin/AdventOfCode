...
begin
   ...
   for Instr of Folds loop
      Fold(Dots, Instr);
      -- For this part, we only need to fold once.
      exit Instruction_Loop;
   end loop Instruction_Loop;

   Count_Row_Loop:
   for Row in Dots.Iterate loop
      Count_Dots_Loop:
      for Dot in Dots(Row).Iterate loop
         Answer := Answer + 1;
      end loop Count_Dots_Loop;
   end loop Count_Row_Loop;

   Put_line("The number of visible dots after completing just the first fold "
            & "instruction is " & Integer'Image(Answer) & ".");
end Day13_1;
