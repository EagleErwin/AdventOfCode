...
procedure Day04_1 is
   ...
   type Play_Field is array (1 .. Number_Of_Players) of Card;
   ...
   procedure Load_File (Numbers : out Drawing;
      ...
      Current_Card     : Card;
   begin
      Cards := (others => (others => (others => (0, False))));
      ...
      while not End_Of_File (Input) loop
         ...
         Cards(Player_Index) := Current_Card;
      end loop;
      Close (File => Input);
   end Load_File;
   ...
   function Check (Player_Card : in Card) return Boolean is
   begin
      -- Check for rows
      Row_Loop:
   ...
   begin
      ...
      for I in Cards'Range loop
         Play(Cards(I), Current_Draw);
      end loop Play_Loop;

      Check_Loop:
      for J in Cards'Range loop
         if Check(Cards(J)) then
            Winner := J;
            exit Check_Loop;
         end if;
      end loop Check_Loop;
...
begin
   ...
   Sum_Of_Unmarked_Numbers := Calculate_Sum(Player_Cards(Winner));
