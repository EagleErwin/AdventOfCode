...
procedure Day18_1 is
   ...
   package Numbers_List is new Vectors(Index_Type   => Natural,
                                       Element_Type => Snailfish_Number_Access);
   use Numbers_List;
   ...
   begin
      ...
      while not End_Of_File (Input) loop
         ...
         Homework.Append(Get_Snailfish_Number(Current_Line));
   ...
   function Sum_All(Values : in Numbers_List.Vector) return Snailfish_Number_Access is
      Result       : Snailfish_Number_Access := Values.First_Element;
      Value_To_Add : Snailfish_Number_Access;
   begin
      Summation_Loop:
      for I in Values.First_Index + 1 .. Values.Last_Index loop
         Value_To_Add := Values(I);
         Result := Sum(Result, Value_To_Add);
         Reduce(Result);
...
begin
   ...
   Result := Sum_All(Homework);

   Answer := Calculate_Magnitude(Result);

   Put_line("The magnitude of the final sum is "
