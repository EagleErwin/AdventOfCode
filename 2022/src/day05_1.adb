   ...
   begin
      ...
      for I in Instructions'Range loop
         ...
         for J in 1 .. Current_Instruction.Amount loop
            ...
            To_Pile_Index   := Playfield(Current_Instruction.To).Nr_Of_Crates + 1;
            Current_Crate := Playfield(Current_Instruction.From).Crates(From_Pile_Index);

            Playfield(Current_Instruction.From).Nr_Of_Crates := From_Pile_Index - 1;
            Playfield(Current_Instruction.To).Nr_Of_Crates := To_Pile_Index;
            Playfield(Current_Instruction.To).Crates(To_Pile_Index) :=
              Playfield(Current_Instruction.From).Crates(From_Pile_Index);
         end loop Move_Loop;
