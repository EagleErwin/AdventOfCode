...
procedure Day07_1 is
   ...
   function Calculate_Answer (Root : in Data_Item_Access) return Integer is
      Answer : Integer := 0;
   begin
      if (Root.Item_Type = Directory) then
         if (Root.Size <= Size_Threshold) then
            Answer := Answer + Root.Size;
         end if;

         Leaves_Loop:
         for N in 1 .. Root.Nr_Of_Items loop
            Answer := Answer + Calculate_Answer(Root.Contents(N));
         end loop Leaves_Loop;
      end if;
...
begin
   ...
   Answer := Calculate_Answer(Filesystem);

   Put_Line("The sum of the total sizes of the directories smaller then or equal to 100000 is " & Integer'Image(Answer));
end Day07_1;
