...
procedure Day11_2 is
   ...
   type Item_Array is array (1 .. Total_Number_Of_Items) of Item_Value;
   type Index_Array is array (1 .. Total_Number_Of_Items) of Integer;
   ...
   Next_Item_ID     : Integer := 0;
   Modulo_Factor    : Integer;
   ...
   begin
      ...
      Monkey.Items(1).ID := Next_Item_ID;
      Monkey.Items(1).Worry := Long_Integer(Current_Item_Worry);
      ...
      while Comma_Pos > 0 loop
         ...
         Next_Item_ID := Next_Item_ID + 1;
         Monkey.Items(Monkey.Nr_Of_Items).ID := Next_Item_ID;
         Monkey.Items(Monkey.Nr_Of_Items).Worry := Long_Integer(Current_Item_Worry);
   ...
   begin
      ...
      while not End_Of_File (Input) loop
         Current_Monkey := (Items => (others => (0 ,0)),
                            Nr_Of_Items => 0,
                            Worry_Updater => (Add, 0),
                            Test_Mod => 1,
                            True_Target => -1,
   ...
   begin
      ...
      for I in 0 .. Number_Of_Monkeys - 1 loop
         ...
         for J in 1 .. Monkeys(I).Nr_Of_Items loop
            Put(Long_Integer'Image(Monkeys(I).Items(J).Worry) & ",");
         end loop Item_Loop;
         Put_Line("");
   ...
   begin
      ...
      for M in Monkeys'Range loop
         ...
         for I in 1 .. Current_Monkey.Nr_Of_Items loop
            ...
            Current_Item := Current_Monkey.Items(I).Worry;
            --Put_Line("Inspecting" & Integer'Image(Current_Item));
            Current_Op  := Current_Monkey.Worry_Updater.Op;
            Current_Arg := Current_Monkey.Worry_Updater.Arg;

            -- Update worry level
            case Current_Op is
               when Add => Monkeys(M).Items(I).Worry := Current_Item + Long_Integer(Current_Arg);
               when Multiply => Monkeys(M).Items(I).Worry := Current_Item * Long_Integer(Current_Arg);
               when Pow => Monkeys(M).Items(I).Worry := Current_Item * Current_Item;
            end case;

            --Put_Line("New worry level:" & Integer'Image(Monkeys(M).Items(I)));
            -- Decrease worry level
            Monkeys(M).Items(I).Worry := Monkeys(M).Items(I).Worry mod Long_Integer(Modulo_Factor);
            --Put_Line("Decreased worry level:" & Integer'Image(Monkeys(M).Items(I)));

            -- Execute test
            Current_Test_Mod := Current_Monkey.Test_Mod;
            if Monkeys(M).Items(I).Worry mod Long_Integer(Current_Test_Mod) = 0 then
            ...
            --Put_Line(Integer'Image(Monkeys(M).Items(I).ID) & " moved from " & Integer'Image(M) & " to " & Integer'Image(Current_Destination));
         end loop Item_Loop;

         Reduce_Monkey_Inventory(Monkeys(M));

         --Print_Inventory(Monkeys);
      end loop Monkey_Loop;
   end Inspect;

   function Find_Monkey_Business return Long_Integer is
      Max_Value : Long_Integer := 0;
      Snd_Value : Long_Integer := 0;

      Current_Value : Long_Integer;
   begin
      for I in Inspection_Count'Range loop
         Current_Value := Long_Integer(Inspection_Count(I));
   ...
   procedure Set_Modulo_Factor (Input : in Input_Array) is
   begin
      Modulo_Factor := 1;
      Find_Mod_Loop:
      for M in Input'Range loop
         Modulo_Factor := Modulo_Factor * Input(M).Test_Mod;
      end loop Find_Mod_Loop;

   end Set_Modulo_Factor;
...
begin
   ...
   Put_Line("The level of monkey business after 10000 rounds is"
