...
procedure Day20_2 is
   ...
   function Count_Distance_At_Least_1000(Input : in Maze) return Integer is
      Minimal_Distance : constant Integer := 1000;
      Result : Integer := 0;
   begin
      for R in Input'Range loop
         for C in Input(R)'Range loop
            if Input(R)(C) /= null and then Input(R)(C).Distance > Minimal_Distance then
               Result := Result + 1;
            end if;
         end loop;
      end loop;
      return Result;
   end Count_Distance_At_Least_1000;
...
begin
   ...
   Answer := Count_Distance_At_Least_1000(Input => Input_Maze);
