...
procedure Day23_1 is
   ...
   procedure Load_File (Nanobots      : out Bot_Array;
                        Strongest_Bot : out Bot_Access) is
      Input           : File_Type;
   ...
   begin
      ...
      while not End_Of_File (Input) loop
         ...
         if Strongest_Bot = null or else Current_Bot.R > Strongest_Bot.R then
            Strongest_Bot := Current_Bot;
         end if;
      end loop Row_Loop;
   ...
   All_Bots      : Bot_Array;
   Strongest_Bot : Bot_Access;
   Answer        : Integer;
begin
   Load_File (Nanobots      => All_Bots,
              Strongest_Bot => Strongest_Bot);

   Answer := Number_Of_Bots_In_Range(Nanobots => All_Bots,
                                     Subject  => Strongest_Bot);

   Put_Line("The strongest bot has a range of" & Integer'Image(Strongest_Bot.R)
            & " and has" & Integer'Image(Answer) & " bots in range.");
end Day23_1;
