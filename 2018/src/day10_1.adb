   ...
   begin
      ...
      if Min_Height < Max_Y - Min_Y then
         ...
         Put_Line("Timestamp:" & Integer'Image(Timestamp - 1)
                  & " Max_X:" & Integer'Image(Max_X)
                  & " Min_X:" & Integer'Image(Min_X)
                  & " Max_Y:" & Integer'Image(Max_Y)
                  & " Min_Y:" & Integer'Image(Min_Y));
         return True;
      end if;
...
begin
   ...
   -- We found the minimal height/width, but therefore we are one ahead of the correct situation.
   -- Calculate again for the previous timestamp.
   Result := Calculate_Positions(Input => Input_Data,
                                 Output => Output_Data,
                                 Timestamp => Current_Time - 1,
                                 Min_Height => Min_Height,
                                 Min_X => Min_X,
                                 Min_Y => Min_Y);
   Print_Output(Input => Output_Data, Min_X => Min_X, Min_Y => Min_Y);
end Day10_1;
