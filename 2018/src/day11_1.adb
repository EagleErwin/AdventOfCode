...
procedure Day11_1 is
   ...
   function Calculate_Area_Power(Power_Supply : in Battery; X, Y : in Coordinate) return Integer is
      Power : Integer := 0;
   begin
      if (X + 2 > 300) or (Y + 2 > 300) then
         return -5 * 9; --Minimal power: invalid
      end if;

      for I in Integer range Y .. (Y + 2) loop
         for J in Integer range X .. (X + 2) loop
   ...
   begin
      for I in Integer range 1 .. 300 loop
         for J in Integer range 1 .. 300 loop
            Current_Power_Level := Calculate_Area_Power(Power_Supply, I, J);
      ...
      return Integer'Image(Best_X) & "," & Integer'Image(Best_Y);
   end Find_Best_Area;
