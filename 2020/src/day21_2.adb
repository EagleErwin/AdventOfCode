...
procedure Day21_2 is
   ...
   package Allergen_Sorter is new Generic_Sorting;
   ...
   -- Map from Allergen to a single Ingredient
   package Allergen_To_Ingr_Map is new Hashed_Maps(Key_Type        => Unbounded_String,
                                                   Element_Type    => Unbounded_String,
                                                   Hash            => Unbounded_String_Hash,
                                                   Equivalent_Keys => "=");
   use Allergen_To_Ingr_Map;
   ...
   -- Returns a vector of sorted allergens
   function Create_Allergen_Vector(Allergens : in Allergen_Map.Map)
                                   return Ingredient_Vector.Vector is
      Result : Ingredient_Vector.Vector;
   begin
      Fill_Loop:
      ...
      Allergen_Sorter.Sort(Result);
      return Result;
   end Create_Allergen_Vector;

   procedure Print_Canonical_Dangerous_Ingredient_List(List      : in Ingredient_Vector.Vector;
                                                       Allergens : in Allergen_Map.Map)  is
      Is_Begin : Boolean := True;
   begin
      Allergen_Loop:
      for A of List loop
         Find_Ingredient_Loop:
         for Cur in Allergens.Iterate loop
            if Key(Cur) = A then
               if Is_Begin then
                  Put(To_String(Allergens(Cur)(0)));
                  Is_Begin := False;
               else
                  Put("," & To_String(Allergens(Cur)(0)));
               end if;
            end if;
         end loop Find_Ingredient_Loop;
      end loop Allergen_Loop;
   end Print_Canonical_Dangerous_Ingredient_List;
   ...
   Allergen_Vect      : Ingredient_Vector.Vector;
...
begin
   ...
   Allergen_Vect := Create_Allergen_Vector(Allergens);

   Put("The canonical dangerous ingredient list is: ");
   Print_Canonical_Dangerous_Ingredient_List(Allergen_Vect, Allergens);
end Day21_2;
