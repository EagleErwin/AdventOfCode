...
procedure Day11_2 is
   ...
   -- Verify if the seat on XS, YS is Occupied. Returns False if it is no seat.
   function Watch (Seats : in Seat_Map;
                   XS : in Integer;
                   YS : in Integer;
                   Count : in out Integer) return Boolean is
   begin
      if Valid_Coordinate(XS, YS, Seats) then
         if Seats(XS)(YS) = Occupied then
            Count := Count + 1;
            return True;
         elsif Seats(XS)(YS) = Empty then
            return True;
         end if;
      else
         return True;
   ...
   function Count_Number_Of_Occupied_Neighbours(Seats : in Seat_Map;
      ...
      Result : Integer := 0;
      Seat_Seen : Boolean := False;
      XS : Integer := X;
      YS : Integer := Y;
   begin
      -- Check top left corner
      while not Seat_Seen loop
         XS := XS -1;
         YS := YS -1;
         Seat_Seen := Watch(Seats, XS, YS, Result);
      end loop;
      ...
      for X in Seats_Copy'Range loop
         ...
         for Y in Seats_Copy(X)'Range loop
            ...
            if Seats_Copy(X)(Y) = Empty and then Neighbours = 0 then
               Seats(X)(Y) := Occupied;
            elsif Seats_Copy(X)(Y) = Occupied and then Neighbours >=5 then
               Seats(X)(Y) := Empty;
            end if;
         end loop Column_Loop;
      end loop Row_Loop;
