   ...
   begin
      ...
      for R in Surface_Grid'Range loop
         ...
         for C in Surface_Grid(R)'Range loop
            if Surface_Grid(R)(C).all /= Initial then
               Min_Y := Integer'Min(Min_Y, R);
               Min_X := Integer'Min(Min_X, C);
      ...
      for Y in Min_Y .. Max_Y loop
         ...
         for X in Min_X .. Max_X loop
            if Surface_Grid(Y)(X).all = White then
               Put('#');
            elsif Surface_Grid(Y)(X).all = Initial then
               Put(' ');
            else
               Put('.');
            end if;
...
begin
   ...
   Print_Grid(Surface_Grid => Ship_Surface);
end Day11_2;
