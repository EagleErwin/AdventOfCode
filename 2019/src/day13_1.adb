...
procedure Day13_1 is
   ...
   -- ### Global variables ### --
   Number_Of_Block_Tiles : Integer := 0;
   ...
   begin
      ...
      loop
         ...
         if Exec.Output_X /= null and then Exec.Output_Y /= null and then Exec.Output_Tile /= null then
            Surface_Grid(Exec.Output_Y.all)(Exec.Output_X.all) := Exec.Output_Tile;
            if Exec.Output_Tile.all = Block then
               Number_Of_Block_Tiles := Number_Of_Block_Tiles + 1;
            end if;
            Reset_Output(Exec);
         end if;
...
begin
   ...
   Print_Grid(Surface_Grid => Game_Surface);

   Put_Line("Total number of block tiles:" & Integer'Image(Number_Of_Block_Tiles));
end Day13_1;
