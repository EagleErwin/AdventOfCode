...
procedure Day19_2 is
   ...
   Grid_Size   : constant Integer := 1_449; -- Max X and negative X value for the grid surface.
   Ship_Size   : constant Integer := 100;
   ...
   -- ### Global variables ### --
   Min_X : Coordinate := 0; -- Smallest X-coordinate in the current row that is in the beam
   Min_Y : Coordinate := 0;
   ...
   function Calculate_Answer(X : in Coordinate; Y : in Coordinate) return Integer is
      Result : Integer := 0;
   begin
      return Integer(X) * 10_000 + Integer(Y);
   end Calculate_Answer;

   function Measure (Intcode : in Application;
                     X       : in Coordinate;
                     Y       : in Coordinate) return Force is
      Intcode_Execution : Execution_Access;
      Measured_Force : Force;
   begin
      Intcode_Execution := new Execution'(Reset (Intcode => Intcode));
      ...
      Log_Debug("Measured at (" & Coordinate'Image(X) & "," & Coordinate'Image(Y) & "): " & Force'Image(Measured_Force));
   ...
   Top_Left_Fits     : Boolean;
   Top_Right_Fits    : Boolean;
   Bottom_Left_Fits  : Boolean;
begin
   Load_File (Intcode => Intcode_App);

   Y_Loop:
   for Y in Ship_Size .. Coordinate'Last loop -- We don't need to check the first 100 lines, it won't fit anyway.
      Log_Debug("Checking row" & Coordinate'Image(Y));
      X_Loop:
      for X in Min_X .. Coordinate'Last loop
         Measured_Force := Measure(Intcode => Intcode_App,
                                   X => X,
                                   Y => Y);
         Environment(Y)(X) := new Force'(Measured_Force);
         if Measured_Force = Pulled then -- Find the first column that is in the beam.
            Min_X := X;
            Top_Left_Fits := True;
            Top_Right_Fits := Pulled = Measure(Intcode => Intcode_App,
                                               X => X + Ship_Size - 1,
                                               Y => Y);
            if not Top_Right_Fits then
               exit X_Loop;
            end if;
            Bottom_Left_Fits := Pulled = Measure(Intcode => Intcode_App,
                                                 X => X,
                                                 Y => Y + Ship_Size - 1);
            if Top_Left_Fits and Top_Right_Fits and Bottom_Left_Fits then
               Put_Line("It fits on line" & Coordinate'Image(Y)
                        & " from " & Coordinate'Image(Min_X)
                        & " until" & Coordinate'Image(X + Ship_Size));
               Min_Y := Y;
               exit Y_Loop;
            end if;
   ...
   Answer := Calculate_Answer(X => Min_X,
                              Y => Min_Y);
